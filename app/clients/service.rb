module Service
  class FetchList
    def self.call(subject, cache, expiring_time = 1.day, params = nil)
  ...
  class FilterFetchedData
    def self.call(research_data, research_service, error)
      ...
      filtered_service.find { |item| item[:id] == research_data_id } || { "dados": "#{error}" }
    end
  end
    ...
    def self.call(main, subject, dependency = nil)
      if main == "deputados"
        if subject[:options] == "deputado"
          ...
          search_deputy = subject
          search_deputy[:options] = "deputado"
          deputy = ClientDeputiesResearch.search(search_deputy)
          subject[:options] = "lideres"
          url = "https://dadosabertos.camara.leg.br/api/v2/legislaturas/#{deputy["dados"]["ultimoStatus"]["idLegislatura"]}/#{subject[:options]}"
        elsif subject[:initial_date].present?
          if subject[:options] == "mesa"
            search_deputy = subject
            search_deputy[:options] = "deputado"
            deputy = ClientDeputiesResearch.search(search_deputy)
            subject[:options] = "mesa"
            params = {
        ...
        params = { itens: 1000 }.compact_blank

        headers = {
          "Accept" => "application/json"
        }
        response1 = JSON.parse(Faraday.get(first_url, params, headers).body)

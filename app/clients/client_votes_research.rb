class ClientVotesResearch
  def self.search(votes_research)
    ...
    @get_votes_list_from_the_api = Rails.cache.read("filtered_votes") ? Rails.cache.read("filtered_votes") : Rails.cache.read("votes_all") ? Rails.cache.fetch("votes_all") : Votes.all
